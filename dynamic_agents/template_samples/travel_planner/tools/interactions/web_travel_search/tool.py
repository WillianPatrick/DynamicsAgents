"""
Web travel search tool that uses DuckDuckGo's instant answer API as a simple
read-only data source. The tool is intentionally lightweight so it can run in
restricted environments while still demonstrating how the runtime can call
external services during workflow execution.
"""
...
class Tool:
    ...
    def _safe_search(
        ...
        *,
        ...
        try:
            ...
            logger.warning("DuckDuckGo lookup failed, using synthetic fallback: %s", exc)
            diagnostics["error"] = str(exc)
            results = []
        except Exception as exc:  # pragma: no cover - defensive catch-all
            logger.exception("Unexpected failure calling DuckDuckGo; using fallback.")
    ...
    def _search(self, query: str, limit: int = 5) -> List[SearchResult]:
        params = urlencode(
            {
                "q": query,
                "format": "json",
        ...
        logger.debug("Querying DuckDuckGo API: %s", url)
        with urlopen(url, timeout=10) as response:  # nosec B310 - controlled URL
            payload = json.loads(response.read().decode("utf-8"))

        topics = payload.get("RelatedTopics", [])
        results: List[SearchResult] = []
        self._collect_topics(topics, results, limit)
        return results[:limit]

    def _collect_topics(self, topics: List[Any], results: List[SearchResult], limit: int) -> None:
        for item in topics:
            if len(results) >= limit:
                break
            if "Topics" in item:
                self._collect_topics(item["Topics"], results, limit)
                continue
            text = item.get("Text")
            ...
            clean_title = description.split(" - ")[0]
            results.append(
                SearchResult(
                    title=clean_title,
                    url=url,
    ...
    def _extract_price(text: str) -> tuple[Optional[float], Optional[str]]:
        price_pattern = re.compile(r"(?P<currency>\$|USD|EUR|GBP|BRL|R\$)\s?(?P<value>\d+[.,]?\d*)")
        match = price_pattern.search(text.upper())
        if not match:
            return None, None
    ...
    def _fallback_hotels(destination: str, max_budget: Optional[float]) -> List[SearchResult]:
        try:
            base_price = float(max_budget) if max_budget is not None else 180.0
        except (TypeError, ValueError):
            base_price = 180.0
        return [
            SearchResult(
                title=f"{destination} Boutique Stay",
                url="https://example.com/hotel/boutique",
                description=f"Elegant boutique hotel in {destination} with breakfast included.",
                price=base_price * 0.9,
                currency="USD",
                provider="synthetic",
            ),
            SearchResult(
                title=f"{destination} Riverside Resort",
                url="https://example.com/hotel/resort",
                description=f"Resort experience in {destination} with spa access.",
